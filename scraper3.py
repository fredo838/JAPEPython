...
class Driver:
  ...
  def generate_rhyming(self,other):
    response = urllib2.urlopen('https://api.datamuse.com/words?rel_rhy='+other)
    html = response.read()
    l = html.split(',')
    rymes = []
    for i in l:
  ...
  def get_words(self,bi_cons,set_words):
    ...
    for c_bi_con in bi_cons:
      ...
      si = int(bi_con[2])-1
      if set_words[fi] == None and set_words[si] == None:
        pass
      elif (not set_words[fi] == None) and (not set_words[si] == None):
        return False,[]
      else:
        if (set_words[fi] == None):
